...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  -p, --project-name    Project name [default: secure-upload]"
...
create_directories() {
    ...
    local dirs=("logs" "backups" "temp" "reports")
...
EOF
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        UPLOAD_ROLE_ARN=$(terraform output -raw upload_role_arn)
        ADMIN_ROLE_ARN=$(terraform output -raw admin_role_arn)
        READONLY_ROLE_ARN=$(terraform output -raw readonly_role_arn)
...
EOF
    ...
    if command -v sudo &> /dev/null; then
        ...
        warn "Could not configure system log rotation (sudo not available)"
...
test_deployment() {
    ...
    if [ ! -f ".env" ]; then
        warn "No .env file found, skipping tests"
        return
    fi
...
EOF
    ...
    echo "You can test uploads with: python3 scripts/secure_upload.py --file $test_file"
}
...
print_summary() {
    ...
    echo "2. Test file uploads: python3 scripts/secure_upload.py --file temp/test_upload.txt"
    echo "3. Start monitoring: python3 monitoring/s3_monitor.py --continuous"
    echo "4. Setup SNS email subscription if not done automatically"
    echo
